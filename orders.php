...
<body>
	...
	<?php 
		$w=$_SESSION['whole'];
		$qw=mysqli_query($dhy,"select * from invoice where wid_s='$w' order by iid desc")or die("Qf1");
		while($data=mysqli_fetch_array($qw))
		{
			$p=$data['pid'];
			$qp=mysqli_query($dhy,"select photo1,m_name,company_brand from products where pid=$p")or die ("QF2");
			$datap=mysqli_fetch_array($qp);

?>

                                <tr  <?php if($data['flag']=="2") { ?>bgcolor="#FFDDDE"<?php } else if($data['flag']=="1") { ?> bgcolor="#D3FEC7" <?php } else { ?> bgcolor="#FFFFFF" <?php } ?>>
...
<?php if($data['flag']=="2") { ?>                                     
<a href="approve.php?a=<?php echo $data['iid'];?>&w=<?php echo $data['wid_s'];?>&p=<?php echo $data['pid'];?>&qty=<?php echo $data['qty'];?>">Approve</a>  
...
<script>
alert("Stock not available for this product");
</script>
<?php } ?>